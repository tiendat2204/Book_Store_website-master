            ...
            while ($fetch_canceled_orders = $canceled_order_query->fetch(PDO::FETCH_ASSOC)) {
                ...
                <li class="canceled-order-item">
                    ...
                    <div class="canceled-order-details">
                        <!-- Hiển thị các chi tiết đơn hàng đã hủy -->
                        ...
                        <!-- Kết thúc hiển thị thông tin sản phẩm đã hủy -->
...
if ($order_query->rowCount() > 0) {
    ...
    <div class="order-row-status">
      ...
      <div class="order-value-status" style="color:<?php echo ($fetch_orders['payment_status'] == 'Đợi xác nhận') ? 'red' : 'green'; ?>"><?php echo $fetch_orders['payment_status']; ?>
     
    
    </div>
    <?php if ($fetch_orders['payment_status'] == 'Đợi xác nhận'): ?>
        <!-- Thêm liên kết hoặc nút để hủy -->
        <a href="./controller/cancel_order.php?order_id=<?php echo $fetch_orders['id']; ?>"
       onclick="return confirm('Bạn có chắc chắn muốn hủy đơn hàng không?');" class="cancel-link">Hủy đơn hàng</a>
    <?php endif; ?>
    </div>
  </div>
</div>
...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        toggleCanceledOrdersButton.addEventListener('click', function () {
            // Toggle (chuyển đổi) lớp CSS để hiển thị/ẩn danh sách
            canceledOrdersList.style.display = canceledOrdersList.style.display === 'none' ? 'flex' : 'none';

            // Thay đổi nội dung của đầu danh sách tùy thuộc vào trạng thái hiện tại
